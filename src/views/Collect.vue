  ...
  <el-card :header="$route.meta.title" shadow="never">
    ...
    >
      ...
      <el-table-column
        ...
        width="120"
      ></el-table-column>
      <el-table-column prop="targetType" label="收藏类型" width="120">
        <template slot-scope="scope">
          <el-tag :type="scope.row.targetType | targetTypeColor">{{
            scope.row.targetType | targetTypeText
          }}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getList(params) {
      ...
      this.$http.get("/admin/collect", { params: input }).then(
        res => {
    ...
    // handleUpdate(row) {},
    handleDelete(row) {
      this.$http.delete(`/admin/collect/${row.id}`).then(
        () => {
          this.$notify({
            title: "成功",
            message: "删除成功",
